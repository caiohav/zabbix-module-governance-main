...
final class QualityJobException extends RuntimeException {}

/**
 * Short-lived quality checkpoints; isolated from availability jobs and their quotas.
 *
 ...
 */
final class QualityJobStore {
    const IDLE_TTL = 300;
    const ABSOLUTE_TTL = 900;
    const MAX_JOB_BYTES = 4194304;
    const MAX_OWNER_JOBS = 8;
    const MAX_JOBS = 32;
    const BUSY = 1;
    const UNAVAILABLE = 2;
    const INVALID = 3;
    ...
    public function __construct(?string $directory = null, ?callable $clock = null) {
        ...
        $directory = $directory ?? sys_get_temp_dir() . DIRECTORY_SEPARATOR
            . 'zabbix-governance-quality-' . substr(hash('sha256', __DIR__), 0, 24);
            ...
            return $this->locked($this->lockName($id), LOCK_EX, function() use ($owner, $id, $initializer): array {
                try {
                    ...
                    $state = self::terminalState([], 'failed',
                        'Cannot start quality analysis. Reload the page and check the saved rules / Não foi possível iniciar a análise. Recarregue a página e confira as regras salvas.');
                }
                $now = $this->now();
        ...
        return $this->change($id, $owner, $expectedSequence, function(array $state) use ($advance): array {
            try {
                ...
                return self::terminalState($state, 'failed',
                    'Analysis interrupted. Previously completed blocks may remain visible; refresh to retry / Análise interrompida. Blocos já concluídos podem permanecer visíveis; atualize para tentar novamente.');
            }
        });
    }
    ...
    /** Only aggregated metrics and bounded examples leave the server; never source rules or host IDs. */
    public static function projection(array $job): array {
        return ['job' => $job['id'], 'sequence' => $job['sequence']]
            + QualityCalculation::projection($job['state']);
    }
    ...
    private static function terminalState(array $state, string $status, ?string $error = null): array {
        $terminal = array_intersect_key($state, array_flip([
            'page', 'revision', 'started_at', 'finished_at', 'progress', 'result'
        ]));
        $terminal['status'] = $status;
        $terminal['progress'] = $state['progress'] ?? [];
    ...
    private function save(array $job): void {
        ...
        // A short first line permits bounded GC inspection without decoding the host scope.
        ...
        if ($header === false || $state === false || strlen($header) + strlen($state) + 10 > self::MAX_JOB_BYTES) {
            throw new QualityJobException('Checkpoint cannot be serialized / Não foi possível serializar a etapa.', self::PAYLOAD);
        }
    ...
    private function makeRoom(array $jobs, string $owner): void {
        ...
        foreach ($owned as $job) {
            ...
            catch (QualityJobException $e) { /* Busy or unsafe files are not eviction candidates. */ }
        }
        ...
        foreach ($candidates as $candidate) {
            ...
            catch (QualityJobException $e) { /* Never wait for a job another request is using. */ }
        }
    }
    ...
    private function locked(string $name, int $mode, callable $operation) {
        ...
        if (!@flock($file, $mode | LOCK_NB)) {
            fclose($file);
            throw new QualityJobException('Calculation busy; retry shortly / Cálculo ocupado; tente novamente em instantes.', self::BUSY);
        }
    ...
    private static function invalidError(): QualityJobException {
        return new QualityJobException('Invalid calculation request / Solicitação de cálculo inválida.', self::INVALID);
    }
    private static function unavailableError(): QualityJobException {
        return new QualityJobException('Calculation unavailable or expired. Start a new calculation / Cálculo indisponível ou expirado. Inicie um novo cálculo.', self::UNAVAILABLE);
    }
    private static function storageError(): QualityJobException {
        return new QualityJobException('Private temporary calculation storage is unavailable / O armazenamento temporário privado do cálculo está indisponível.', self::STORAGE);
    }
}
