...
const tests = [
    ...
    ['mixed daily chart uses item availability and coverage and refuses native or department daily fabrication', async () => {
        ...
        assert.equal(daily.series[2].name, 'Cobertura'); assert.equal(daily.series[2].yAxisIndex, undefined);
        assert.equal(daily.series[0].type, 'bar'); assert.equal(daily.series[2].type, 'line');
        ...
        assert.equal(daily.yAxis.length, 1); assert.equal(daily.xAxis.length, 1);
        assert.equal(daily.series[0].itemStyle.color({value: 99.8}), '#be3c3c');
        assert.equal(daily.series[0].itemStyle.color({value: item.target}), '#1f774b');
        assert.equal(daily.series[0].itemStyle.color({value: null}), p.colors.muted);
    ...
    ['JSON export v3 preserves explicit sources, null durations and original precision', async () => {
        for (const name of ['native', 'mixed', 'unavailable', 'timezone', 'calendar', 'item_unknown', 'escaped', 'items_only']) {
            ...
            assert.equal(payload.format, 'governance-availability-v3'); assert.equal(payload.module_version, '1.25.1');
