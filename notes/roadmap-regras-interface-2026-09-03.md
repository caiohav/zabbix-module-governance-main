...
### Continuação 1.18.0 — Editor de disponibilidade compacto

- Usuário informou ter instalado 1.17.0 e depois respondeu “deu certo” ao teste solicitado de busca/prévia; o escopo preciso do teste não foi detalhado. Tentativas de acesso do agente ao Edge foram bloqueadas pela verificação de URL do Computer Use. Não tratar como QA autenticado realizado pelo agente nem insistir sem necessidade.
- Retomado item 6 do roadmap: verificações por item passam a details, com chave, número e janela no resumo. Salvas recolhidas; novas abertas. Remoção permanece dentro do corpo. Ajuda de chave/evidência, combinação, pesos/fontes e SLA fica recolhida. Importação opcional de IDs do SLA também recolhida. Campos UP/DOWN e regras mantidos.
- Participação de cada peso no total configurado do departamento aparece no resumo da tecnologia, com duas casas no máximo. É informação visual, não SLA nem peso efetivo de meses incompletos. Qualquer peso inválido no departamento torna a participação “—”; outros departamentos não são afetados. Não persiste campos novos nem consulta API.
- Tests/availability-config.js agora tem 17 cenários, incluindo preservação do JSON ao recolher, validação abrindo details, texto de chave seguro, nova verificação aberta, participação e isolamento entre departamentos/PT/EN. Regressões PHP/JS completas passaram.
