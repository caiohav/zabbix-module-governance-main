...
<div class="gov-container gav <?= !empty($data['is_dark']) ? 'gov-theme-dark' : '' ?>" id="gav-dashboard" data-lang="<?= $pt ? 'pt' : 'en' ?>" data-timezone="<?= $e($data['config']['timezone']) ?>">
    ...
    <?php if (!empty($data['rules_changed'])): ?><p class="gav-notice gav-no-print" id="gav-rules-changed" role="status"><?= $data['config']['departments']
        ? $t('As regras foram alteradas depois que este cálculo começou. O relatório antigo foi ocultado e não será continuado. Clique em “Calcular mês” para usar as regras salvas atualmente.', 'Rules changed after this calculation started. The old report is hidden and will not be resumed. Click “Calculate month” to use the currently saved rules.')
        : $t('As regras foram alteradas depois que este cálculo começou. O relatório antigo foi ocultado. Configure um departamento antes de calcular novamente.', 'Rules changed after this calculation started. The old report is hidden. Configure a department before calculating again.') ?></p><?php endif ?>
    <p class="gav-notice gav-no-print" id="gav-idle-help" <?= $report || $job || !empty($data['rules_changed']) || !$data['config']['departments'] ? 'hidden' : '' ?>><?= $t('Selecione a competência e clique em Calcular mês. As fontes serão consultadas em etapas curtas. Para SLA nativo, selecione um mês encerrado; o relatório só aparece após a conclusão.', 'Select the month and click Calculate month. Sources will be queried in short stages. For native SLA, select a closed month; the report only appears after completion.') ?></p>
    ...
    <details class="gav-help gav-info-panel"<?= $observedPolicy ? ' id="gav-observed-policy"' : '' ?>><summary><span class="gav-info-icon" aria-hidden="true">i</span><?= $t('Entenda o cálculo', 'Understand the calculation') ?></summary><div class="gav-info-content">
        ...
        <?php if ($observedPolicy): ?><li><?= $t('Qualquer servidor fora: uma queda confirmada basta; hosts desconhecidos são ignorados enquanto outro tem estado conhecido. Média dos servidores: média dos percentuais observados de cada host com dados. Todas as verificações continuam obrigatórias para confirmar disponibilidade por host ou no serviço combinado.', 'Any host down: one confirmed outage is enough; unknown hosts are ignored while another has a known state. Mean of hosts: mean of the observed percentages of hosts with data. All checks remain required to confirm availability per host or in a combined service.') ?></li><?php endif ?>
    ...
    <section class="gav-department gav-<?= $status($ds, $department['target']) ?>">
        ...
        foreach ($department['technologies'] as $ti => $tech): $s = $metric($tech); $isObservedItem = $observedPolicy && ($tech['source'] ?? 'items') === 'items'; ?>
            <tr><th><a href="#gav-tech-<?= $di ?>-<?= $ti ?>" class="gav-open-tech"><?= $e($tech['name']) ?></a><small><?= ($tech['source'] ?? 'items') === 'sla' ? $t('SLA nativo · mensal', 'Native SLA · monthly') : (($tech['check_scope'] ?? '') === 'selected_hosts' ? $t('Itens · hosts por verificação', 'Items · host per check') : ($tech['mode'] === 'any_down' ? $t('Itens · qualquer servidor fora', 'Items · any host down') : $t('Itens · média dos servidores', 'Items · mean of hosts'))) ?></small></th>
                ...
                <td><?= $e($percent($tech['target'])) ?></td><td><?= $e($percent($s['coverage'])) ?><?php if (isset($tech['data_quality'])): ?><small><?= $e($tech['data_quality']['hosts_with_data']) ?> / <?= $e($tech['hosts_total']) ?> <?= ($tech['check_scope'] ?? '') === 'selected_hosts' ? $t('serviço com dados', 'service with data') : $t('hosts com dados', 'hosts with data') ?></small><?php endif ?></td>
        ...
        <details class="gav-details gav-tech-detail" id="gav-tech-<?= $di ?>-<?= $ti ?>"><summary><?= $e($tech['name']) ?> · <?= ($tech['source'] ?? 'items') === 'sla' ? $t('SLA nativo', 'Native SLA') : (($tech['check_scope'] ?? '') === 'selected_hosts' ? $t('serviço combinado', 'combined service') : count($tech['hosts']) . ' ' . (count($tech['hosts']) === 1 ? $t('host avaliado', 'host assessed') : $t('hosts avaliados', 'hosts assessed'))) ?></summary>
            ...
            <?php if (isset($tech['data_quality'])): $quality = $tech['data_quality']; ?><p class="gav-data-quality"><?= ($tech['check_scope'] ?? '') === 'selected_hosts' ? $t('Serviço com estado conhecido:', 'Service with known state:') : $t('Hosts com estado conhecido:', 'Hosts with known state:') ?> <strong><?= $e($quality['hosts_with_data']) ?> / <?= $e($tech['hosts_total']) ?></strong> · <?= $t('Sem estado conhecido:', 'Without known state:') ?> <strong><?= $e($quality['hosts_without_data']) ?></strong>.</p>
            ...
            <?php endif ?>
            <?php if ($observedPolicy && isset($tech['observation'])): $observation = $tech['observation']; ?><p class="gav-muted"><?= ($tech['check_scope'] ?? '') === 'selected_hosts'
                ? $t('Os itens dos hosts escolhidos formam um serviço: qualquer DOWN conta como queda; sem DOWN, todos precisam estar UP.', 'Items from the selected hosts form one service: any DOWN counts as downtime; without DOWN, every check must be UP.')
                : ($tech['mode'] === 'any_down'
            ...
            <?php endif ?>
            <?php foreach ($tech['warnings'] as $warning): ?><p class="gav-notice gav-error"><?= $e($message($warning)) ?></p><?php endforeach ?>
            <div class="gav-table-scroll"><table class="list-table gav-table gav-hosts-table"><thead><tr><th><?= ($tech['check_scope'] ?? '') === 'selected_hosts' ? $t('Serviço', 'Service') : 'Host' ?></th><th><?= $observedPolicy ? $t('Disponibilidade observada', 'Observed availability') : $t('Disponibilidade', 'Availability') ?></th><th><?= $t('Cobertura', 'Coverage') ?></th><th><?= $t('Indisponível', 'Down') ?></th><th><?= $t('Desconhecido', 'Unknown') ?></th><th><?= $t('Itens / observações', 'Items / notes') ?></th></tr></thead><tbody>
                <?php foreach ($tech['hosts'] as $hi => $host): $sourceWarnings = []; $hs = $metric($host); ?><tr><th><?= $e($host['name']) ?></th><td><?= $e($percent($hs['score'])) ?><?php if ($observedPolicy && $hs['score'] === null): ?><small><?= $t('Sem estado conhecido', 'No known state') ?></small><?php endif ?></td><td><?= $e($percent($hs['coverage'])) ?></td><td><?= $e($duration($hs['down'])) ?></td><td><?= $e($duration($hs['unknown'])) ?></td><td>
                    <?php foreach ($host['sources'] as $source): $trendSource = ($source['data_source'] ?? 'history') === 'trends_conservative'; ?><div class="gav-source"><code><?= $e($source['key']) ?></code><?php if (($tech['check_scope'] ?? '') === 'selected_hosts'): ?><small><?= $t('Host:', 'Host:') ?> <?= $e($source['host_name'] ?? '') ?> · ID <?= $e($source['hostid'] ?? '') ?></small><?php endif ?><small class="gav-source-state"><strong><?= $trendSource ? $t('Trends horárias conservadoras', 'Conservative hourly trends') : $t('Histórico detalhado', 'Detailed history') ?></strong></small>
                        <?php if (!$trendSource): ?>
