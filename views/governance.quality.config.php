...
$dashboardUrl = 'zabbix.php?action=governance.quality.view' . ($returnPage !== '' ? '&page=' . rawurlencode($returnPage) : '');
...
<div class="gov-container gqp gqp-editor <?= !empty($data['is_dark']) ? 'gov-theme-dark' : '' ?>" id="gov-config" data-lang="<?= $pt ? 'pt' : 'en' ?>" data-conflict="<?= !empty($data['conflict']) ? '1' : '0' ?>">
    ...
    <details class="gqp-help"><summary><?= $t('Como configurar suas métricas', 'How to configure your metrics') ?></summary>
        <ul>
            <li><?= $t('Selecione os hosts usando Todas (E), Qualquer (OU) ou uma expressão personalizada. Adicionar/Editar abre a condição em uma janela; Aplicar altera somente o rascunho e Cancelar descarta a edição da janela. Depois escolha o que medir nesses hosts.', 'Select hosts using All (AND), Any (OR), or a custom expression. Add/Edit opens the condition in a dialog; Apply only changes the draft and Cancel discards the dialog edit. Then choose what to measure in those hosts.') ?></li>
            <li><?= $t('O botão Testar usa as condições já aplicadas no rascunho, sem salvar, e mostra até 50 hosts; o total considera todos os selecionados. Salvar todas as páginas grava as alterações.', 'Test uses the conditions already applied to the unsaved draft and shows up to 50 hosts; the total includes all selected hosts. Save all pages persists the changes.') ?></li>
            ...
            <li><?= $t('Na medição por tags, informe nomes ou aliases separados por vírgula. Sem valores aceitos, qualquer valor não vazio é considerado conforme. Nas condições de seleção, a comparação de nome/valor da tag é exata.', 'For tag metrics, enter names or aliases separated by commas. Without accepted values, any non-empty value is compliant. Host-selection conditions compare the tag name/value exactly.') ?></li>
            <li><?= $t('Grupos por nome podem incluir subgrupos conforme a opção escolhida: Equipes pode incluir Equipes/Banco de Dados. Um ID sempre seleciona apenas o grupo exato.', 'Named groups can include subgroups depending on the selected option: Teams can include Teams/Database. An ID always selects only the exact group.') ?></li>
