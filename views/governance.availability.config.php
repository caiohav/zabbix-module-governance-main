...
<div class="gov-container gav <?= !empty($data['is_dark']) ? 'gov-theme-dark' : '' ?>" id="gav-config" data-lang="<?= $pt ? 'pt' : 'en' ?>">
    <section class="gav-report-settings" aria-labelledby="gav-settings-title">
        ...
        <details class="gav-setting-help"><summary><?= $t('Sobre o tratamento de dados ausentes', 'About missing data handling') ?></summary>
            ...
            <p class="gav-muted"><?= $t('Todas as verificações continuam obrigatórias por host ou pelo serviço de hosts escolhidos. Chaves ausentes ou validade não resolvida ainda exigem atenção. Ausência de dados não vira disponibilidade nem queda.', 'Every check remains required per host or for the service built from selected hosts. Missing keys or unresolved validity still require attention. Missing data is not treated as availability or downtime.') ?></p>
        </details>
    </section>
    ...
    <details class="gav-help"><summary><?= $t('Como o cálculo funciona e quais são os limites', 'How the calculation works and its limits') ?></summary>
        <ul>
            <li><?= $t('Na fonte por itens, sem host específico as verificações são aplicadas a cada host do grupo. Com host definido em todas as verificações, os itens escolhidos são combinados como um único serviço, mesmo em hosts diferentes. Os hosts devem pertencer aos grupos configurados.', 'With the item source, checks without a specific host apply to each host in the group. When every check selects a host, the chosen items combine as one service even across hosts. Those hosts must belong to the configured groups.') ?></li>
            ...
            <li><?= $t('Por host ou serviço combinado: todas as verificações em UP confirmam disponibilidade; qualquer DOWN confirma indisponibilidade; sem queda e com alguma verificação desconhecida, o estado permanece desconhecido.', 'Per host or combined service: all checks UP confirm availability; any DOWN confirms downtime; with no outage and any unknown check, the state remains unknown.') ?></li>
